...
struct DLL_LINKAGE Point {
	...
	Point(const Vec3f&Pos, const Vec3f &Normal, const Vec3f&RGB) { pos = Pos; normal = Normal; rgb = RGB; }
	Point(const Vec3f&Pos, const Vec3f &Normal, const Vec3f&RGB, int picnum) { pos = Pos; normal = Normal; rgb = RGB; image = picnum; }
...
public:
	...
	//新添加的函数
	void SetGroupNum(int &num) { GroupNum_plane = num; };
	int GetGroupNum() { return GroupNum_plane; };
